...
{
    public partial class MultiTakeView : UserControl
    {
        IMultiTakeable takeable;
        Button title;
        Button[] btns;
        List<string> pageInfos;
        SortedSet<(int, int)> takes;
        int curPageNumber;
        int pagesLength;
        ...
        int CurPage
        {
            ...
            set
            {
                ...
                for (int i = 0; i < numInPage; i++)
                    if (takes.Contains((curPageNumber, i)))
                        btns[i].BasedChoosing();
                    else
                        btns[i].BasedChoice();
        ...
        public MultiTakeView(IMultiTakeable take)
        {
            InitializeComponent();
            takeable = take;
            ...
            for (int i = 0; i < numInPage; i++)
            {
                ...
                btns[i].Click += (a, b) =>
                {
                    if (takes.Contains((curPageNumber, m)))
                    {
                        btns[m].BasedChoice();
                        takes.Remove((curPageNumber, m));
                    }
                    else
                    {
                        btns[m].BasedChoosing();
                        takes.Add((curPageNumber, m));
                    }
                };
            ...
            OKBtn.Based();
            OKBtn.Click += (a, b) => takeable.Choose(takes);
            OKBtn.Text = "确定";
        }
